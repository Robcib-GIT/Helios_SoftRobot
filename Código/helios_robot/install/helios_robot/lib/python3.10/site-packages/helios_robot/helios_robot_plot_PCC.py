...
from numpy import cos, sin, pi, sqrt, arange, deg2rad, nan_to_num
import numpy as np
...
class PccRosNode(Node):
    def __init__(self):
        ...
        self.subscription = self.create_subscription(
            ...
            self.sub_callback,
            10)
        self.pose_publisher = self.create_publisher(Float32MultiArray, '/helios_pose_meas', 10)
        self.theta = np.zeros(6)
        self.phi = np.zeros(6)
        self.L = [6.3, 6.3, 6.3, 6.3, 6.3, 6.3]
    ...
    def sub_callback(self, msg):
        ...
        if data.size >= 12:
            theta_deg = data[:6]
            phi_deg = data[6:12]
            theta_deg = nan_to_num(theta_deg, nan=0.0)
            phi_deg = nan_to_num(phi_deg, nan=0.0)
    ...
    def timer_callback(self):    
        ...
        self.ax.set_zlim(l, 0)
        plt.draw()
        plt.pause(0.01)
        if knots:
            last_point = knots[-1]
            msg = Float32MultiArray()
            msg.data = [float(last_point[0]), float(last_point[1]), float(last_point[2])]
